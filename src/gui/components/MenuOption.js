...
export default class OptionList extends GUIElement{
  ...
  constructor (game,text,menuConfig,callback,className,numberOfOptions) {
    super(game)
    this.menuConfig=menuConfig;
    //init(text,callback,className,numberOfOptions);
    ...
    var txt = new Phaser.Text(
      this.game,
      x,
      (numberOfOptions * 80) + y,
      text,
      style.navitem[className]
    );

    this.getRenderGroup().add(txt);
