...
export default class NineImage {
  ...
  public static create(aGame: Phaser.Game,
    aWidth: number, aHeight: number,
    aKey: string, aFrame: number | string,
    aTop: number = 0,
    aLeft: number = 0,
    aBottom: number = 0,
    aRight: number = 0,
    aRepeats: boolean = false): Phaser.BitmapData {            // image
      NineImage._image = aGame.cache.getImage(aKey);            // get frame
      ...
      NineImage.calculateNineImage(aWidth, aHeight, aTop, aLeft, aBottom, aRight, aRepeats);                        // create nine image bitmap data            NineImage._nineImage = new Phaser.BitmapData(aGame, "NineImage" + (NineImage._textureKey++), NineImage._width, NineImage._height);            // render into bitmap data            NineImage.renderNineImage();                             // return result            return NineImage._nineImage;
    }

    // -------------------------------------------------------------------------

    private static calculateNineImage(aWidth: number,
      aHeight: number,
      aTop: number,
      aLeft: number,
      aBottom: number,
